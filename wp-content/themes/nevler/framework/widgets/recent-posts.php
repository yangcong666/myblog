...
class nevler_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'nevler_rp', // Base ID
			__('Nevler Recent Posts with Thumbnails', 'nevler'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'nevler' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'nevler' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'nevler' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','nevler' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'nevler' ); ?></label>
...
add_action( 'widgets_init', 'register_nevler_widget' );  
function register_nevler_widget() {  
    register_widget( 'nevler_Recent_Posts' );  
}  
