 ...
 * 'oxygen' feature with remove_post_type_support().
 *
 * @since 0.4.0
 * @return void
 */
function theme_layouts_add_post_type_support() {
	...
	foreach ( $post_types as $type )
		add_post_type_support( $type, 'oxygen' );
}
...
function theme_layouts_remove_post_type_support() {
	...
	if ( function_exists( 'bbp_get_reply_post_type' ) )
		remove_post_type_support( bbp_get_reply_post_type(), 'oxygen' );
}
...
function theme_layouts_get_layouts() {
	...
	$layouts = get_theme_support( 'oxygen' );
...
function theme_layouts_strings() {
	...
	$strings = array(
		...
		'1c'      => __( 'One Column', 'oxygen' ),
		'2c-l'    => __( 'Two Columns, Left', 'oxygen' ),
		'2c-r'    => __( 'Two Columns, Right', 'oxygen' ),
		'3c-l'    => __( 'Three Columns, Left', 'oxygen' ),
		'3c-r'    => __( 'Three Columns, Right', 'oxygen' ),
		'3c-c'    => __( 'Three Columns, Center', 'oxygen' )
	);
...
function theme_layouts_admin_setup() {
	...
	/* If the attachment post type supports 'oxygen', add form fields for it. */
	if ( post_type_supports( 'attachment', 'oxygen' ) ) {
...
/**
 * Adds the theme layouts meta box if the post type supports 'oxygen' and the current user has 
...
function theme_layouts_add_meta_boxes( $post_type, $post ) {
	...
	if ( ( post_type_supports( $post_type, 'oxygen' ) ) && ( current_user_can( 'edit_post_meta', $post->ID ) || current_user_can( 'add_post_meta', $post->ID ) || current_user_can( 'delete_post_meta', $post->ID ) ) )
		add_meta_box( 'theme-layouts-post-meta-box', __( 'Layout', 'oxygen' ), 'theme_layouts_post_meta_box', $post_type, 'side', 'default' );
}
...
function theme_layouts_post_meta_box( $post, $box ) {
	...
	<div class="post-layout">
		...
		<p><?php _e( 'Layout is a theme-specific structure for the single view of the post.', 'oxygen' ); ?></p>
...
function theme_layouts_customize_register( $wp_customize ) {
	...
	if ( true === $args['customize'] ) {
		...
		$wp_customize->add_section(
			'layout',
			array(
				'title'      => esc_html__( 'Layout', 'oxygen' ),
				'priority'   => 190,
		...
		$wp_customize->add_control(
			...
			array(
				'label'    => esc_html__( 'Global Layout', 'oxygen' ),
				'section'  => 'layout',
