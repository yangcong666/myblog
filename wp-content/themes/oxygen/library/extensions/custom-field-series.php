...
/* Add support for the 'oxygen' extension to posts. */
...
function custom_field_series( $args = array() ) {
	...
	if ( !empty( $meta_value ) ) {
		...
		if ( !empty( $series_posts ) ) {
			...
			$series .= '<h4 class="series-title">' . apply_filters( 'custom_field_series_title', __( 'Articles in this series', 'oxygen' ) ) . '</h4>';
			$series .= '<ul>';
...
/**
 * Adds post type support of 'oxygen' to the 'post' post type.  Developers should register support 
