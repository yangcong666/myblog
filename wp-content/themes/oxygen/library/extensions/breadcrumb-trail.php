...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			...
			'home'                => __( 'Home',                    'oxygen' ),
			'search'              => __( 'Search results for "%s"', 'oxygen' ),
			'error_404'           => __( '404 Not Found',           'oxygen' ),
			'paged'               => __( 'Page %d',                 'oxygen' ),
			'archives'            => __( 'Archives',                'oxygen' ),
			'archive_minute_hour' => __( 'g:i a',                   'oxygen' ),
			'archive_minute'      => __( 'Minute %d',               'oxygen' ),
			'archive_hour'        => __( 'g a',                     'oxygen' ),
			'archive_day'         => __( 'd',                       'oxygen' ),
			'archive_week'        => __( 'Week %d',                 'oxygen' ),
			'archive_month'       => __( 'F',                       'oxygen' ),
			'archive_year'        => __( 'Y',                       'oxygen' ),
		//	'edit'                => __( 'Edit',                    'oxygen' ), // @todo Implement edit link
		);
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'oxygen' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'oxygen' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'oxygen' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
