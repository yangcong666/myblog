...
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Summerleaf_Home_Imggrid_Widget");'));

// The widget class
class Summerleaf_Home_Imggrid_Widget extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'home_imggrid_widget', 'description' => esc_html__( "在首页左侧内容栏中显示夏叶主题文章图片列表", 'summerleaf') );
		parent::__construct('sl_block4', esc_html__('夏叶首页文章列表风格二', 'summerleaf'), $widget_ops);
		$this->alt_option_name = 'widget_home_imggrid';
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_home_imggrid', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
...
//
		...
		$custom_query = new WP_Query( apply_filters( 'widget_home_imggrid_args', $custom_query_args ) );
      ...
      echo '</div><div class="clear"></div></div>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_home_imggrid', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function remove_cache() {
		wp_cache_delete('widget_home_imggrid', 'widget');
	}
