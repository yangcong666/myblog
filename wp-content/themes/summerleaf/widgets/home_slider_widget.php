...
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Summerleaf_Home_Slider_Widget");'));

// The widget class
class Summerleaf_Home_Slider_Widget extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'home_slider_widget', 'description' => esc_html__( "在首页左侧内容栏中显示夏叶主题文章幻灯片", 'summerleaf') );
		parent::__construct('sl_block5', esc_html__('夏叶首页幻灯片一', 'summerleaf'), $widget_ops);
		$this->alt_option_name = 'widget_home_slider';
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_home_slider', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
...
//
		...
		$custom_query = new WP_Query( apply_filters( 'widget_home_slider_args', $custom_query_args ) );
    ...
    echo '<div id="slider" class="flexslider"><ul class="slides">';
				...
				while ( $custom_query->have_posts() ) : $custom_query->the_post();
					...
					?> 
					  <li>  
              <a href="<?php echo $post_link; ?>"><img src="<?php echo get_post_featured_image($post->ID,'700*300');?>" alt="<?php echo $post_title; ?>" /></a>
              <div class="slider-caption" style="bottom: 12px;">
              	<h2><a target="_blank" href="<?php echo $post_link; ?>"><?php echo $post_title; ?></a></h2></div>
            </li>       
    ...
    echo '</ul></div>'; 
      ...
      echo '</div>
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_home_slider', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function remove_cache() {
		wp_cache_delete('widget_home_slider', 'widget');
	}
