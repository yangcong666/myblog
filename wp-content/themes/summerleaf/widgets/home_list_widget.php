...
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Summerleaf_Home_List_Widget");'));

// The widget class
class Summerleaf_Home_List_Widget extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'home_list_widget', 'description' => esc_html__( "在首页左侧内容栏中显示夏叶主题文章图文列表", 'summerleaf') );
		parent::__construct('sl_block7', esc_html__('夏叶首页文章列表风格三', 'summerleaf'), $widget_ops);
		$this->alt_option_name = 'widget_home_list';
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_home_list', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	  ...
	  ob_start();
		...
		$custom_query = new WP_Query( apply_filters( 'widget_home_list_args', $custom_query_args ) );
       ...
       echo '<div class="cat-site">';                               
					...
					?>  
     <article id="post-<?php echo $post->ID; ?>" <?php post_class(); ?> data-wow-delay="0.3s"> 
      <figure class="thumbnail"> 
       ...
       <span class="cat"><?php echo get_the_category_list(' ', '', $post->ID); ?></span> 
      </figure> 
      <header class="entry-header"> 
       <h2 class="entry-title"><a href="<?php echo $post_link; ?>" rel="bookmark"><?php echo $post_title; ?></a></h2> 
      </header> 
      <!-- .entry-header --> 
      <div class="entry-content"> 
       ...
       <span class="entry-meta"> <span class="date"><i class="fa fa-calendar-check-o"></i> <?php echo get_the_date('Y/m/d', $post->ID); ?> </span> <span class="comment"> <a href="<?php echo $post_link; ?>#comments" rel="external nofollow"><i class="fa fa-comment-o"></i> <?php echo esc_html($post->comment_count); ?></a></span> </span> 
       <div class="clear"></div> 
      </div> 
      <!-- .entry-content --> 
      <span class="entry-more"><a href="<?php echo $post_link; ?>" rel="bookmark">阅读全文</a></span> 
     </article>
      ...
      echo '</div>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_home_list', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function remove_cache() {
		wp_cache_delete('widget_home_list', 'widget');
	}
	...
	public function form( $instance ) {
		...
		<p>
		   ...
		   <label for="<?php echo $this->get_field_id('thumbnail'); ?>"><?php esc_html_e('是否分页', 'summerleaf') ?></label>
	  </p>	  
		<p>
