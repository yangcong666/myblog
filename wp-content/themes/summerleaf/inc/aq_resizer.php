		...
		public function process($url, $width = null, $height = null, $crop = null, $single = true, $upscale = false) {
			...
			if (!$dims && ( ( ( null === $height && $orig_w == $width ) xor ( null === $width && $orig_h == $height ) ) xor ( $height == $orig_h && $width == $orig_w ) )) {
				...
				// Else, we resize the image and return the new resized image url.
					...
					$resized_img_path = $this->resize($img_path, $width, $height, $crop); // Fallback foo.
...
function get_post_featured_image($post_id, $alias, $show_cap = true) {
        $img_src = wp_get_attachment_image_src(get_post_thumbnail_id($post_id), 'single-post-thumbnail');
        $img_src = $img_src[0];
        $url = get_image($img_src, $alias, $show_cap);
        return $url;
    }


function resize_image($img_src, $alias, $show_cap = true) {
        return get_image($img_src, $alias, $show_cap);
    }

function get_image($img_src, $alias, $show_cap = true) {
        if (empty($alias)) {
            return $img_src;
        }
        ...
        if (!$new_img_src) {
            if ($show_cap) {
                return 'http://placehold.it/' . $al[0] . 'x' . $al[1] . '&amp;text=SUMMERLEAF';
            }
        }
