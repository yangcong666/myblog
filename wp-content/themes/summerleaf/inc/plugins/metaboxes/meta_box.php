...
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/inc/plugins/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'post_checkboxes':
			$posts = get_posts( array( 'post_type' => $post_type,'orderby'=> 'post_name','order'=>'ASC', 'posts_per_page' => -1, 'suppress_filters' => 0 ) );
			echo '<ul class="meta_box_items">';
			foreach ( $posts as $item ) 
		...
		break;
		        //feature_checkboxes
        case 'feature_checkboxes':
            $posts = array( 'photo','nature','food','panda','family','hiking','culture','kungfu','history','cruise','exclusive','minority','sunrise','sunset','landscape','winter','portrait','wildlife','tea','rafting','golf','festival','cycling','climbing' );
            echo '<ul >';
            foreach ( $posts as $item )
                echo '<li style="float: left; width:150px; overflow: hidden; margin-right: 10px "><input type="checkbox" value="' . $item . '" name="' . esc_attr( $name ) . '[]" id="' . esc_attr( $id ) . '-' . $item . '"' , is_array( $meta ) && in_array( $item, $meta ) ? ' checked="checked"' : '' , ' />
						<label for="' . esc_attr( $id ) . '-' . $item . '">' . $item . '</label></li>';

            
            echo '</ul> ' . $desc , ' &nbsp;';
            break;
        //location_checkboxes
        case 'location_checkboxes':
            $posts = get_posts( array( 'post_type' => $post_type,'orderby'=> 'post_name','order'=>'ASC', 'posts_per_page' => -1, 'suppress_filters' => 0 ) );
            echo '<ul >';
            foreach ( $posts as $item )
                echo '<li style="float: left; width:150px; overflow: hidden; margin-right: 10px "><input type="checkbox" value="' . $item->ID . '" name="' . esc_attr( $name ) . '[]" id="' . esc_attr( $id ) . '-' . $item->ID . '"' , is_array( $meta ) && in_array( $item->ID, $meta ) ? ' checked="checked"' : '' , ' />
						<label for="' . esc_attr( $id ) . '-' . $item->ID . '">' . $item->post_title . '</label></li>';

            $post_type_object = get_post_type_object( $post_type[0] );
            echo '</ul> ' . $desc , ' &nbsp;<div class="description" style="clear: both"><a href="' . admin_url( 'edit.php?post_type=' . $post_type[0]) . '">Manage ' . $post_type_object->labels->name . '</a></div>';
            break;
		// post_drop_sort
		case 'post_drop_sort':
			//areas
		...
		case 'tax_checkboxes':
			...
			echo '<span class="description">' . (isset($field['desc']) ? $field['desc'] : '') . ' <a href="' . admin_url('edit-tags.php?taxonomy=' . $tax_id) . '">Manage ' . $taxonomy->label . '</a></span>';
		break;
		// date
		case 'date':
