 ...
 * @since 1.0
 */
function navolio_light_get_standard_fonts() {
	return apply_filters(
		'navolio_light_standard_fonts_array', array(
 ...
 * @since 1.0
 */
function navolio_light_enqueue_google_font( $font ) {
	...
	$subsets     = '';
	$get_subsets = navolio_light_get_options( array('font_subsets', array( 'latin' ) ) );
	...
	// Weights
	$weights = apply_filters( 'navolio_light_font_weights', array( '300', '400', '500', '700', '800' ) );
	...
	// Enqueue style
	wp_enqueue_style( 'navolio-light-google-font-' . $handle, $url, array(), false );
}

function navolio_light_enqueue_google_font_url( $font ) {
	// Get list of all Google Fonts
	$google_fonts = navolio_light_get_google_fonts_array();
