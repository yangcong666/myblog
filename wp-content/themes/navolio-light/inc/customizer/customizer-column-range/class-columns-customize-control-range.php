<?php
/**
 * Customizer Control: columns-range-control.
 *
...
class Navolio_Light_Columns_Customize_Range_Control extends WP_Customize_Control {
	/**
	 * @access public
	 * @var string
	 */
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'range-columns-ui', get_theme_file_uri( '/inc/customizer/customizer-column-range/assets/range-value-ui.css' ) );
	...
	public function render_content() {
		...
		<select id="js-widgets-num-<?php echo sanitize_html_class( $this->id ); ?>" style="margin-bottom: 1em;">
		<?php for ( $i=0; $i <= (int)$attrs['maxCols']; $i++ ) {
			printf( '<option value="%1$d"%2$s>%1$d</option>', wp_kses_post( $i ), selected( $i, $widgets_num ) );
		} ?>
		</select>
		<div id="js-layout-<?php echo sanitize_html_class( $this->id ); ?>"></div>
		<script>
			jQuery( function( $ ) {
				'use strict';
				...
				$( '#js-widgets-num-<?php echo sanitize_html_class( $this->id ); ?>' ).on( 'change', function ( ev ) {
					var $el = $( '#js-layout-<?php echo sanitize_html_class( $this->id ); ?>' );
