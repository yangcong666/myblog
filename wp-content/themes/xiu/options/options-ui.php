...
function opshui_page() {
    ...
    <div id="opshui-metabox" class="metabox-holder">
		...
		<div class="opshui-sidebar"><div class="opshui-sidebar-widget"><a href="http://www.thefox.cn/url/aliyun" target="_blank"><img src="http://www.thefox.cn/opad.png"></a></div></div>
</div>
<script type="text/javascript">
window.onload = loadpopup();　　
function openwin() {
	　　window.open("http://www.thefox.cn/url/aliyun")　　
}
...
function loadpopup() {
　
	if (get_cookie('options_aotoclick') == '') {

		　　openwin()

		　　document.cookie = "options_aotoclick=yes"	　　
	}　　
}
</script>
...
function opshui_validate( $input ) {
	...
	foreach ( $options as $option ) {
		...
		if ( has_filter( 'hui_sanitize_' . $option['type'] ) ) {
			if( isset($input[$id]) ) $clean[$id] = apply_filters( 'hui_sanitize_' . $option['type'], $input[$id], $option );
		}
	}
