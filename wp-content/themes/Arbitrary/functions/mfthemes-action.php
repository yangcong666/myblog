 ...
 */
	if($_POST['action'] == 'mfthemes_ajax_comment' && 'POST' == $_SERVER['REQUEST_METHOD']){
		...
		if ( empty($post->comment_status) ) {
			...
			mfthemes_ajax_err(__('Invalid comment status.')); // 將 exit 改為錯誤提示
		}
		...
		if ( !comments_open($comment_post_ID) ) {
			...
			mfthemes_ajax_err(__('评论已关闭!')); // 將 wp_die 改為錯誤提示
		} elseif ( 'trash' == $status ) {
			...
			mfthemes_ajax_err(__('Password Protected')); // 將 exit 改為錯誤提示
		} else {
		...
		if ( $user->exists() ) {
			...
			if ( get_option('comment_registration') || 'private' == $status )
				mfthemes_ajax_err(__('你必须要登陆之后才可以发表评论.')); // 將 wp_die 改為錯誤提示
		}
		...
		if ( get_option('require_name_email') && !$user->exists() ) {
			if ( 6 > strlen($comment_author_email) || '' == $comment_author )
				mfthemes_ajax_err( __('请填写昵称和邮箱.') ); // 將 wp_die 改為錯誤提示
			elseif ( !is_email($comment_author_email))
				mfthemes_ajax_err( __('请填写一个有效的邮箱.') ); // 將 wp_die 改為錯誤提示
		}

		if ( '' == $comment_content )
			mfthemes_ajax_err( __('请输入评论.') ); // 將 wp_die 改為錯誤提示
		...
		if ( $wpdb->get_var($dupe) ) {
			mfthemes_ajax_err(__('您已经发布过一条相同的评论!'));
		}
		...
		if ( $flood_die ) {
			mfthemes_ajax_err(__('请过一会再发表评论.'));
			}
		}
		...
		if ( $edit_id ){
			...
			if(current_user_can("edit_comment",$edit_id)){
				...
				if($current_user->user_email==$comment_author_email){
					...
					mfthemes_ajax_err("您不能修改为他人评论！");
				}
		 
			}else{
				mfthemes_ajax_err("您没有权限编辑该评论！");
			}
		} else {
...
function mfthemes_ajax_pagenavi(){ // pagenavi
	if( isset( $_GET['action'] ) && $_GET['action']== 'mfthemes_ajax_pagenavi' ){
		...
		if(!$postid || !$pageid){
			mfthemes_ajax_err(__('Error post id or comment page id.'));
		}
		// get comments
		...
		if(!$comments){
			mfthemes_ajax_err(__('Error! can\'t find the comments'));
		}
